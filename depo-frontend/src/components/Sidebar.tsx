...
import {
    Package2, LayoutDashboard, Package, ArrowLeftRight,
    Users, Settings, LifeBuoy, Crown, Warehouse,
    ClipboardList, GitBranch, Truck, Wallet, FileStack
} from 'lucide-react';
            ...
            <div className="flex h-full max-h-screen flex-col gap-2">
                ...
                <div className="flex h-16 items-center border-b border-slate-100 px-6">
                    <Link href="/dashboard" className="flex items-center gap-2 font-bold text-xl">
                        <div className="bg-blue-600 p-1.5 rounded-lg"><Package2 className="h-5 w-5 text-white" /></div>
                        <span className="text-slate-900">SaaS</span><span className="text-blue-600">Depo</span>
                    </Link>
                </div>
                    ...
                    <nav className="grid items-start px-4 text-sm font-medium lg:px-6 space-y-1">
                        ...
                        <div className="pb-4">
                            ...
                            <Link href="/dashboard" className={`${linkClass} ${isActive('/dashboard')}`}>
                                <LayoutDashboard className="h-4 w-4" /> Özet Ekranı
                            </Link>

                            <Link href="/warehouses" className={`${linkClass} ${isActive('/warehouses')}`}>
                                <Warehouse className="h-4 w-4" /> Depolarım
                            </Link>
                            ...
                            <Link href="/transactions" className={`${linkClass} ${isActive('/transactions')}`}>
                                <ArrowLeftRight className="h-4 w-4" /> Stok Hareketleri
                            </Link>

                            <Link href="/stock-forms" className={`${linkClass} ${isActive('/stock-forms')}`}>
                                <FileStack className="h-4 w-4" /> Toplu İşlem (Fişler)
                            </Link>

                            <Link href="/purchase-orders" className={`${linkClass} ${isActive('/purchase-orders')}`}>
                                <Truck className="h-4 w-4" /> Sipariş & Mal Kabul
                            </Link>

                            <Link href="/finance" className={`${linkClass} ${isActive('/finance')}`}>
                                <Wallet className="h-4 w-4" /> Finans
                            </Link>

                            <Link href="/suppliers" className={`${linkClass} ${isActive('/suppliers')}`}>
                                <Truck className="h-4 w-4" /> Tedarikçiler
                            </Link>

                            <Link href="/requests" className={`${linkClass} ${isActive('/requests')}`}>
                                <ClipboardList className="h-4 w-4" /> Talepler
                            </Link>
                        </div>
                        ...
                        {canSeeManagement && (
                            <div className="pb-4">
                                ...
                                {canManageBranches && (
                                    <Link href="/branches" className={`${linkClass} ${isActive('/branches')}`}><GitBranch className="h-4 w-4" /> Şubeler</Link>
                                )}
                ...
                <div className="mt-auto p-4 border-t border-slate-100 bg-slate-50/50">
                    <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                        <div className="flex items-center gap-2 text-blue-700 font-semibold text-sm mb-1"><LifeBuoy className="h-4 w-4" /> Destek</div>
                        <p className="text-xs text-blue-600/80">v3.0.0 Enterprise</p>
                    </div>
                </div>
            </div>
        </div>
